...
To extract usable features from the textual content more robustly, we make use of Anthropic's \texttt{claude-opus-4-6} generative language model. The model is used to read the text of the agenda, read the text of the minutes, and to extract structured information about the proposed project and about the decisions of the Commission. Our approach follows a growing literature in economics that uses large
language models to extract structured data from unstructured text
\citep[e.g.,][]{ash2023text, korinek2023generative, ziems2024can, dell2025deep}. The methodology and prompts we used to perform the data extraction are described in detail in the appendix, and are robust to the use of alternative language models.\footnote{The full data extraction pipeline, including all prompts and scraping code, is also publicly available at \url{https://github.com/ed-kung/lacpc}.}  In this section, we will simply focus on the data features that were extracted from the published texts.
...
\item The type of case (residential development, mixed-use development, non-residential development, or other);
...
\item Type of author: whether the author of the document is an individual, a public official, a professional representing an interested party (e.g. lawyers, consultants), or a community organization;
